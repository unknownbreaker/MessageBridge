...
public struct TailscaleDevice: Sendable, Identifiable, Codable {
    ...
    public init(id: String, name: String, ipAddresses: [String], online: Bool, os: String?, isSelf: Bool) {
        self.id = id
        self.name = name
...
public actor TailscaleManager {
    ...
    public func openTailscaleApp() async {
    ...
    /// Open Tailscale download page in browser
    public func openDownloadPage() async {
    ...
    private func parseDevicesFromJSON(_ json: String) -> [TailscaleDevice] {
        ...
        do {
            guard let dict = try JSONSerialization.jsonObject(with: data) as? [String: Any],
                  let peerDict = dict["Peer"] as? [String: Any] else {
                return []
            }
    ...
    private func parseDevice(from dict: [String: Any], isSelf: Bool) -> TailscaleDevice? {
        guard let id = dict["ID"] as? String ?? dict["PublicKey"] as? String,
              ...
              let ips = dict["TailscaleIPs"] as? [String] else {
            return nil
        }
