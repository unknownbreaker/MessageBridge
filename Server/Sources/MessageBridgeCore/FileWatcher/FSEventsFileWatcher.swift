...
public actor FSEventsFileWatcher: FileWatcherProtocol {
  ...
  public func startWatching(handler: @escaping @Sendable () -> Void) async throws {
    ...
    serverLog(
      "Starting to watch directory \(directoryPath) for changes to \((path as NSString).lastPathComponent)"
    )
      ...
      DispatchQueue.global(qos: .utility).async { [weak self] in
        ...
        if let stream = stream {
          FSEventStreamScheduleWithRunLoop(
            stream, CFRunLoopGetCurrent(), CFRunLoopMode.defaultMode.rawValue)
