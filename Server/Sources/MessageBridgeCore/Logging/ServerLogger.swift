...
public struct ServerLogEntry: Sendable {
  ...
  public init(
    timestamp: Date = Date(), level: ServerLogLevel, message: String, file: String,
...
public final class ServerLogger: @unchecked Sendable {
  ...
  /// Log a message
  public func log(
    _ level: ServerLogLevel, _ message: String, file: String = #file, function: String = #function,
    line: Int = #line
  ) {
    let entry = ServerLogEntry(
      level: level, message: message, file: file, function: function, line: line)
